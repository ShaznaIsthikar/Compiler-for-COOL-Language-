//Kanishka Dilhan E/17/065
//Ishini Udara E/17/312
...
extern char *curr_filename;         /* used in cgen */
...
ClassTable::ClassTable(Classes classes) : semant_errors(0) , error_stream(cerr) {
    ...
    //start a scope and 
    //add all the classes to the table considering the scope
    ...
    for ( int i = classes->first(); classes->more(i); i = classes->next(i) ) {
        ...
        if(class_name == SELF_TYPE || class_name == Object || class_name == Int || 
            ...
            //check multiply is defined or not
            ostream& os =  semant_error(current_class);
            os << "Class " << class_name << " was previously defined." << endl;
            continue;
        } 

        //add defined class to the table
        class_symtable.addid(class_name,current_class);
    }

    //check for valid feature definition
    for( int i = classes->first(); classes->more(i); i = classes->next(i) ){
        current_class = (c_node)classes->nth(i);
    ...
    if ( class_symtable.probe(Main) == NULL){
        ...
        //looking main method if main class is define
...
//valid features checking
void ClassTable::semant_class(c_node current_class){
	// each class with it own feature table
	...
	//check the type if feature is attribute or method
...
void ClassTable::semant_method(c_node current_class,method_class* method){
	...
	//give error for incompatible return type
	...
	// if no errors, add to the table
    current_table.addid(method_name,method);
}



//type checking for features
void ClassTable::semant_class_attr(c_node current_class){
	...
	//get the feature from the table and type check for attributes and methods
    for( int i = features->first(); features->more(i); i = features->next(i) ){
        Feature feature = features->nth(i);
...
void ClassTable::semant_attr_expr(c_node current_class,attr_class* attr){
    ...
    //check attribute type and the expression type is compatible
...
//type checking for formal list , return type and body of method
void ClassTable::semant_method_expr(c_node current_class,method_class* method){
	...
	//formal list type check
    for( int i = formals->first(); formals->more(i); i = formals->next(i) ){
        Formal f = formals->nth(i);
        semant_formal(current_class,f);
    }
	
	//check body of a method
    Expression expr = method->get_expr();
    ...
    //check function type and its return
...
void ClassTable::semant_formal(c_node current_class,Formal f){
    ...
    //add to feature table if no errors
...
void ClassTable::semant_expr(c_node current_class,Expression expr){
    ...
    switch (expr->get_type()){
        ...
        case static_dispatchType:
            {
				...
				// find the method and throw an error for types which are not defined
				...
				for(i = ens->first() ; ens->more(i); i = ens->next(i)){
					...
					if(formals->more(i)){
                        ...
                        //type compatibility of formal defintion
		...
		//boolean and arithmatic operation type check
        case plusType:
            {
