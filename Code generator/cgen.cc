...
int label_num = 0;                                  // for label indices of the program
std::vector<Symbol> let_variables;                  // for let vairiables
std::map<Symbol,int> arg_list;                      // argument list of current method
Symbol present_class;                               // current class
std::map<Symbol, std::map<Symbol, int> > attrTab;   // for attributes of all classes
std::map<Symbol, std::map<Symbol, std::pair<int, Symbol> > > dispTab;  // methods of all classes
...
// function to get number of arguments
...
// function to dispatch all arguments on the stack
// checks for method being called
void dispatch_util(ostream& s, Expressions actual, Expression expr)
{
  for(int i=actual->first(); actual->more(i); i = actual->next(i))
  {
    actual->nth(i)->code(s);
    emit_push(ACC,s);
    let_variables.push_back(No_type); // let variables can find the right offset
  }
  expr->code(s);
...
void static_dispatch_class::code(ostream &s) {
  ...
  present_class = type_name;  // taking the class being called upon
  ...
  emit_jalr(T1,s);            // going back to the address of the method
  for(int i=0; i<num_of_arg; i++)
  {
    let_variables.pop_back(); // let variables can find the right offset
  }
  present_class = t;
}
...
void cond_class::code(ostream &s) {
  ...
  pred->code(s);                  // evaluating condition expression
  emit_load(T1,3,ACC,s);
  emit_beqz(T1,else_label,s);     // checking whether it is false
  then_exp->code(s);              // then expression implementation
  int endif_label = label_num++;
  emit_branch(endif_label,s);     // jump to end of if statement
  emit_label_def(else_label,s);
  else_exp->code(s);              // else expression implementation
...
void loop_class::code(ostream &s) {
  ...
  emit_load(T1,3,ACC,s);                // checking whether it is false
  emit_beqz(T1,loop_end_label,s);
  body->code(s);                        // loop body implementation
  emit_branch(loop_condition_label,s);  // jump to check loop condition
...
void block_class::code(ostream &s) {
  ...
  {
    body->nth(i)->code(s);              // generate code for each expression
  }
}

void let_class::code(ostream &s) {
  init->code(s);
  if(init->get_type() == NULL)          // if it's a basic type...
  {
    if(type_decl == Int)
    {
...
void object_class::code(ostream &s) {
  ...
  if(arg_list.find(name) != arg_list.end()) // if it's a passed argument...
  {
    emit_load(ACC,(arg_list.size() - arg_list[name] + 2),FP,s);
  }
  else                                      //if it's an attribute of a object...
  {
...
void CgenClassTable::code_class_nameTab()
{
  ...
  for(int i = classes_vector.size()-1;i>-1;i--) // iterating over all classes in the vector
  {
